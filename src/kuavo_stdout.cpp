...
void MainWindow::panelRosbag() {
    ...
    textOutput->append(">>> [" + QTime::currentTime().toString() + "] 正在导出stdout...");
    ...
    std::string rosDir = std::string(homeDir) + "/.ros/stdout";
    ...
    try {
        ...
        if (result.empty()) {
            QMessageBox::information(this, "提示", "未找到任何文件夹位于 ~/.ros/stdout/ 。");
            return;
        }

        std::string destDir = std::string rosDir = std::string(homeDir) + "/Documents/lejurobot_stdout/";
        ...
        textOutput->append(">>> [" + QTime::currentTime().toString() + "] stdout 导出完成...");
        QMessageBox::information(this, "完成", "日志文件导出成功！");
